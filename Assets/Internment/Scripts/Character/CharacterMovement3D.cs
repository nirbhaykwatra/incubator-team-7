...
public class CharacterMovement3D : CharacterMovementBase
{
    ...
    private Color GetOxygenBarColor(float value)
    {
        return Color.Lerp(Color.gray, Color.cyan, Mathf.Pow(value / MaxOxygen, 2));
    }
    ...
    public override void TryApplyDamage(float damage)
    {
        if (!Invincible) ApplyDamage(damage);
    }
